...
# Function to create the Elasticsearch index with kNN settings for similarity search and stemming for standard search
def create_index():
        ...
        body={
            "retriever": {
                "rrf": {
                    ...
                    "rank_constant": 1,  # You can adjust the rank constant as needed
                    "rank_window_size": 50  # Adjust window size if necessary
                }
            },
            "size": rsize  # Limit the number of results returned
        },
        explain=True
    )
    ...
    if user_query:
        ...
        if search_results:
            for result in search_results:
                ...
                st.write(f"Page Number: {doc['metadata']['page_number']}")
                st.write(result["_score"]) 
                st.write(result["_explanation"]["description"]) 
                st.write("---")
        else:
