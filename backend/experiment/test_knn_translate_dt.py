...
INDEX_NAME = "chatbot_knn_translate_dt"
...
UPLOADS_FOLDER = "uploads_knn_translate"
...
def app():
    ...
    if uploaded_files:
        for uploaded_file in uploaded_files:
            ...
            if chunks:
                document_list = asyncio.run(chunk_processing(chunks, uploaded_file.name))  # Process the chunks into documents
    ...
    if user_query:
        ...
        if search_results:
            for result in search_results:
                ...
                st.write(f"Page Number: {doc['metadata']['page_number']}")
                st.write(((1 / result["_score"]**(1/2)) - 1))  # Display similarity distance
                st.write("---")
        else:
